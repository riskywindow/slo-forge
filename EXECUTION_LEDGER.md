...
| GenesisCapsule, artifacts, sandbox and independent validation | Genesis trust lane | exercised on macOS; Linux/Windows unexercised | shared `main` checkout | capsule/artifacts/sandbox | Genesis IR | tamper/stale/hardware/sandbox/replay tests | capsule and sandbox evidence | `38999aa` through `e52868b` |
...
| State and memory synthesis and migration verification | state lane | exercised | shared `main` checkout | state transformations/runtime | IR/runtime | ownership/memory/rollback/migration tests | state fixtures | `9636331`, `f9bffcf` |
| Search, lifecycle, budgets, Pareto archive and CEGIS | synthesis lane | exercised locally | shared `main` checkout | search/synthesis | verification/surfaces | deterministic budget/lifecycle/CEGIS and demos | candidate histories/counterexamples | `aded57e`, `fb7e36f`, `e52868b` |
...
| Autopsy-guided mutation and frozen regions | Autopsy lane | mutation mapping/guards exercised; comparative campaign absent | shared `main` checkout | Autopsy bridge | search/Autopsy | focused mapping/freeze tests | mutation budgets | `5c7a114` |
| Optimization lineage, transfer and invalidation | lineage lane | mechanics exercised; performance hypothesis not evaluated | shared `main` checkout | lineage/CLI | lifecycle/CEGIS | SQLite/transfer/invalidation demo | `artifacts/genesis/lineage-transfer-demo/` | `01683d2`, `ac43dde` |
| Champion-challenger evolution and promotion safety | evolution lane | local fixture exercised; external live promotion disabled | shared `main` checkout | evolution | capsule/runtime/recovery | 29 focused tests plus evolution demo | evolution timelines/gates | `16fa7a4`, `e9fa92c` |
| Executable red team and minimization | red-team lane | exercised | shared `main` checkout | redteam/counterexamples | verification/CEGIS | `make genesis-redteam-demo` | 19-family corpus/constraints | `91e2bd4`, `d3363ae` |
...
| Flagship HybridDecoder and artifact-backed demos | flagship lane | exercised CPU-only | shared `main` checkout | model/demo/reports | trusted core/evolution | Genesis, zero-day and evolution demos | `artifacts/genesis/{demo,zero-day-demo,evolution-demo}/` | `f60f9b1`, `1e86a0b` |
...
| Evaluation H1-H9 and statistical review | evaluation lane | reports generated; H6 in-scope supported, H1/H3/H7/H8 partial, H2/H4/H5/H9 not evaluated | shared `main` checkout | evaluation | demos | `make genesis-evaluation` and self-validator | `artifacts/genesis/evaluation/` | `a36eb28`, `e52868b` |
