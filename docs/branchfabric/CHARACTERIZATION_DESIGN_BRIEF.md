...
The completed corpus validates the trace and analysis path, but it does **not** justify building a BranchFabric device yet. The best-supported result is that Helix state sharing is real at the software/content-addressing level, while the observed CPU control-path operations have low end-to-end Amdahl leverage. The primary four-branch fixture allocated 2,660 model-state bytes instead of 10,640 bytes under naive independent allocation, for 75% sharing efficiency and 1.0 physical amplification (`SYNTHETIC` workload, host byte accounting; `artifacts/branchfabric/characterization/vertical-seed-41-final/sharing-analysis.json`). In the same fixture, making all traced COW handling free would improve the modeled full transaction by only 1.00345x (`SYNTHETIC/HARDWARE_BACKED_REAL`; `artifacts/branchfabric/analysis/amdahl/cpu-reference.json`).
...
The realistic control flow is the deterministic Helix coding-agent fixture, not production traffic. Its canonical seed-41 trace contains 61 branch events and 17 state events with zero dropped and zero filtered events; model-state sizes are explicitly simulated, while event timing is measured on the Apple host (`SYNTHETIC/HARDWARE_BACKED_REAL`; `artifacts/branchfabric/characterization/vertical-seed-41-final/vertical-run.json`). The measurement was repeated at seeds 73 and 113, with the same artifact structure (`SYNTHETIC/HARDWARE_BACKED_REAL`; `artifacts/branchfabric/characterization/vertical-seed-73/vertical-run.json` and `artifacts/branchfabric/characterization/vertical-seed-113/vertical-run.json`).

The broader matrix declares coding-agent, reasoning, verification-heavy, capacity-reclamation, and highly-divergent classes across controlled fanout, context, suffix, environment, pressure, migration, and fault dimensions. These cells are sweep inputs with `distribution_claim: false`; they are not asserted production distributions (`SYNTHETIC`; `benchmarks/branchfabric/characterization.yaml`). The separate Continuum lifecycle has 18 state events, zero drops, measured host operations, and deterministic simulated-network spans (`SYNTHETIC`, mixed `HARDWARE_BACKED_REAL`/`SIMULATED_HARDWARE`; `artifacts/branchfabric/continuum/seed-41/continuum-trace-run.json`).

## 3. Branch DAG characteristics

The measured Helix branch group has fanout 4, with three pruned children and one successful child (`SYNTHETIC/HARDWARE_BACKED_REAL`; `artifacts/branchfabric/analysis/workload/helix-coding-agent-seed-41.json`). Branch lifetimes are tightly clustered: p50 384.576 ms and p99 385.363 ms across four branches; fork-to-first-private-state ranges from 5.690 ms to 253.467 ms. The derived shared-root reference lifetime is 339.063 ms and peak refcount is 4 (`SYNTHETIC/HARDWARE_BACKED_REAL`; same artifact).
...
The canonical workload analyzer observes 13,300 model-segment operation bytes and 6,376 unique filesystem bytes, but its `model_state_bytes` field is zero because v1 segment identity does not split the simulated model capsule into KV/recurrent/sampler components (`SYNTHETIC`; `artifacts/branchfabric/analysis/workload/helix-coding-agent-seed-41.json`). Database, process-reconstruction, transaction, integrity, and runtime-reconstructible byte distributions remain unknown. Actual KV size, HBM residency, padding, TP layout, and allocator overhead are unavailable until a compatible runtime is measured.

## 5. Sharing characteristics

For four siblings, shared logical model bytes total 8,844 and physical allocation is 2,660 bytes against 10,640 bytes of naive independent state. That yields 75% sharing efficiency and 1.0 physical amplification (`SYNTHETIC`; `artifacts/branchfabric/characterization/vertical-seed-41-final/sharing-analysis.json`). The two-branch Continuum fixture independently records 2,727 shared logical bytes, 449 private bytes per branch, 3,176 physical bytes versus 6,352 naive bytes, 50% sharing efficiency, and 1.0 amplification (`SYNTHETIC`; `artifacts/branchfabric/continuum/seed-41/sharing-analysis.json`).

Environment sharing differs materially: the live Helix workspace eagerly restores each branch, so 594 base bytes become 2,376 live bytes across four branches with no claimed filesystem block sharing (`SYNTHETIC`; `artifacts/branchfabric/characterization/vertical-seed-41-final/sharing-analysis.json`). The content-addressed checkpoint layer deduplicates objects, but CAS bytes are not physical filesystem blocks. A future architecture must keep model-state sharing, live environment sharing, and checkpoint deduplication as separate mechanisms.

## 6. Divergence behavior

All four captured trajectories diverge at token index 0 and action index 0 (`SYNTHETIC`; `artifacts/branchfabric/analysis/workload/helix-coding-agent-seed-41.json`). The branches nevertheless retain their common captured state until private writes occur, with the measured first-private times noted in Section 3. This is an intentional immediate-divergence case and cannot establish early-, middle-, late-, or production-weighted divergence distributions.
...
The Helix fixture records four logical COW events, 1,303 bytes written, and 668 old bytes replaced. It records no page-fault counter and therefore does not claim zero actual page faults (`SYNTHETIC/HARDWARE_BACKED_REAL`; `artifacts/branchfabric/characterization/vertical-seed-41-final/sharing-analysis.json`). The Continuum fixture records 512 COW bytes over eight logical pages (`SYNTHETIC`; `artifacts/branchfabric/continuum/seed-41/sharing-analysis.json`).
...
Only one canonical transformation chain is observed: a single `STATE_RESHARD` over 3,434 bytes, with 1.779 ms measured host latency in the seed-41 analysis (`SYNTHETIC/HARDWARE_BACKED_REAL`; `artifacts/branchfabric/analysis/transform/continuum-seed-41.json`). No repack, transpose, quantize, dequantize, compression, decompression, or encryption chain occurs in that corpus. Temporary allocation and memory-read/write counters are absent.
...
The state lifecycle records 8,087 source payload bytes and 8,087 logical delivery bytes with zero retry payload bytes (`SYNTHETIC`, mixed host-copy and `SIMULATED_HARDWARE`; `artifacts/branchfabric/analysis/transport/continuum-seed-41.json`). One in-process host-memory transfer moves 3,434 bytes with 69 microseconds recorded transfer time; three modeled transfers account for 4,653 bytes and use deterministic simulated latency (`SYNTHETIC`, respectively `HARDWARE_BACKED_REAL` and `SIMULATED_HARDWARE`; same artifact).
...
Every observed `STATE_SEND` has fanout 1. The conservative multicast detector finds zero opportunities and zero exact payload-reduction bytes for thresholds from 2 through 128 destinations; four sends remain ungrouped because v1 lacks a shared non-empty payload-dependency identity (`SYNTHETIC`, mixed host/simulated transport; `artifacts/branchfabric/analysis/transport/continuum-seed-41.json`). This supports `NOT_JUSTIFIED` for `STATE_MULTICAST` in the current bounded corpus, not the stronger assertion that production Helix can never benefit.
...
Migration is absent from the full Helix lifecycle and is measured separately by Continuum (`SYNTHETIC`; `artifacts/branchfabric/characterization/vertical-seed-41-final/vertical-run.json`). The Continuum trace includes snapshot, publish, fork, logical COW/append/delta, reshard, transfer, commit, and reclaim; twelve operation timings are host-real and six transport timings are simulated (`SYNTHETIC`, mixed timing classes; `artifacts/branchfabric/continuum/seed-41/continuum-trace-run.json`).

In the Amdahl model, free reshard produces at most 1.02349x migration speedup, free simulated transfer 1.01008x, free delta extraction 1.00574x, and free commit 1.00076x (`SYNTHETIC`, host timings except `SIMULATED_HARDWARE` transfer; `artifacts/branchfabric/analysis/amdahl/cpu-reference.json`). These bounds apply only to the tiny fixture. Pre-copy convergence, recopy waste, dirty rate over time, failure recovery, and physical bytes crossing a real device or network remain unknown.
...
In the traced seed-41 waterfall, the largest observed stage union is reward at 316.409 ms; fork is 46.300 ms, checkpoint is 19.544 ms, capture is 13.968 ms, and summed rollout work is 10.148 ms (`SYNTHETIC/HARDWARE_BACKED_REAL`; `artifacts/branchfabric/analysis/workload/helix-coding-agent-seed-41.json`). These are inclusive spans; an exclusive critical path is explicitly unavailable. Production, branch-ready, migrate, resume, and evaluation stages are not observed in that single trace.
...
For the full Helix fixture, traced COW is 0.3438% of modeled duration and free COW yields at most 1.00345x; reclamation is 0.2481% and free reclamation yields at most 1.00249x (`SYNTHETIC/HARDWARE_BACKED_REAL`; `artifacts/branchfabric/analysis/amdahl/cpu-reference.json`). For the separate migration fixture, reshard has the largest measured leverage at 2.2955% and the free-operation bound is 1.02349x; the other bounds are listed in Section 12 (`SYNTHETIC`, mixed timing classes; same artifact).
...
In the bounded corpus, `STATE_QUANTIZE`, `STATE_DEQUANTIZE`, `STATE_COMPRESS`, `STATE_DECOMPRESS`, `STATE_ENCRYPT`, `STATE_DECRYPT`, and `STATE_MULTICAST` are `NOT_JUSTIFIED` because no qualifying operations or opportunities occur (`SYNTHETIC`; `artifacts/branchfabric/requirements/branchfabric_requirements.json`). Hardware SHA/checksum is also not recommended because its critical-path fraction is not established, despite a measured host baseline.
...
The only defensible minimum is depth 1 for each observed state operation, because maximum recorded concurrency is 1. Recommended depth, pathological maximum, and backpressure policy are all unknown (`SYNTHETIC`; `artifacts/branchfabric/requirements/branchfabric_requirements.json`). This is a conformance minimum, not a hardware sizing recommendation.
...
Do not start an FPGA target. The current corpus neither supplies device-locality/bandwidth requirements nor shows a material end-to-end software bottleneck. The highest current free-operation migration bound is only 1.02349x on a tiny host-state fixture (`SYNTHETIC/HARDWARE_BACKED_REAL`; `artifacts/branchfabric/analysis/amdahl/cpu-reference.json`).
...
Do not start a SmartNIC/DPU implementation. There is no physical multi-node trace, all observed sends have fanout 1, and the pre-copy link is simulated (`HARDWARE_BACKED_REAL` capability status plus `SYNTHETIC/SIMULATED_HARDWARE` trace; `artifacts/branchfabric/hardware/status-seed-20260809.json` and `artifacts/branchfabric/analysis/transport/continuum-seed-41.json`).
...
- Measurement bias: one deterministic coding-agent fixture may encode the intended architecture more strongly than real workloads. The immediate divergence and fanout-4 result must not be generalized (`SYNTHETIC`; `artifacts/branchfabric/analysis/workload/helix-coding-agent-seed-41.json`).
- Instrumentation distortion: full tracing has a paired median wall-time change of -0.126%, which is within the measured noise floor and is not a speedup claim; full-trace CPU median is 2.101 s versus 2.030 s disabled (`SYNTHETIC/HARDWARE_BACKED_REAL`, six measured samples per level; `artifacts/branchfabric/overhead/helix-cpu-3seed-2rep-v4/instrumentation-overhead.json`).
