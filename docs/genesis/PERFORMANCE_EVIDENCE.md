...
| H2 and H9 | `sloforge.genesis.whole-stack-campaign/v1` at `campaigns/whole-stack/report.json` | Real candidate, genome, policy, transformation, tensor-rewrite, and Fabric-fixture artifacts evaluated by a deterministic CPU service model. Service units are not wall time or hardware measurements. |
