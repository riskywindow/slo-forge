...
The DSL provides the mechanics for admission, batching, scheduling, cache, migration and recovery decisions when their inputs/outputs are encoded as scalars. The current local fixture exercises a request/serving batching policy; it does not claim that every listed policy family is already connected to a production runtime or that generated Rust source is emitted. The trusted target implemented today is bytecode plus the checked interpreter.
