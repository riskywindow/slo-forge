...
Kernel synthesis is evidence-targeted rather than an attempt to rewrite attention. The focused CPU lab targets the HybridDecoder quantized persistent-state update from a measured synthetic microprobe, generates restricted candidates, and checks strides, aliasing, non-finite values, and boundaries in the sandbox. It then patches one content-addressed reference package with the generated kernel and scalar adapter, re-inspects it, generates reference and candidate streaming runtimes, and runs randomized paired interleaved traces. An independent sandbox replay checks exact token and typed final-state semantics and reconstructs statistics from raw samples. This is local CPU end-to-end evidence only; GPU and production-hardware claims remain unavailable.
