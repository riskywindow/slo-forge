...
SLOForge's simulator follows the tradition of queueing/digital-twin tools such as Vidur: measured phase curves drive faster-than-real-time discrete events. Its distinctive requirement is that the same artifacts feed compiler selection, gateway policy, chaos replay and a hash-verified evidence report. The optimizer uses conventional monotonic interpolation, coordinate-stratified split-conformal residual calibration, exhaustive/random/successive-halving-style baselines and Pareto dominance. Novelty is not claimed for those algorithms individually; the checked-in calibration samples are small and empirical held-out coverage is reported rather than assumed.

Kubernetes, Modal and Truss already materialize and operate serving applications. SLOForge is not a replacement control plane for any of them. Its emitters select engine-specific load/generate paths for Transformers, vLLM, SGLang, TensorRT-LLM and explicit mock plans and validate what can be validated without credentials. They still lower only a subset of each provider's semantics: billing, identity, secrets, lifecycle, distributed engine topology and the full canary/rollback contract remain with the deployment environment.
