...
raw samples, robust summary, environment, hardware fingerprint, status, and an
explicit failure reason. Raw samples carry trial index, warmup/measurement phase,
duration, optional throughput, and seed. Summaries include median, p95, p99,
median absolute deviation, and a seeded bootstrap interval.
...
for NCCL tests and other optional transport tools. Missing CUDA, GPU, NCCL,
DeepEP, NIXL, UCX, or RDMA capability produces a skipped/unavailable record; no
CPU or TCP fallback is relabeled as the requested primitive.
...
Warmups are retained but excluded from summaries. A benchmark has at least three
measurement samples; bootstrap summaries use at least 100 rounds. Synthetic
noise uses the explicit seed. Actual benchmarks enforce a timeout and fixed
payload size. Placement and command are part of the evidence, so two results are
not pooled unless their environment and physical binding are compatible.

## Known limitations

No NVIDIA GPU, NVLink, NCCL, InfiniBand, or RoCE measurements were executed on
the current machine. `artifacts/fabric-demo/fabric-profile-raw/profile.json` is
synthetic calibrated input for the CPU flagship demo and must not be quoted as
