...
The full checkout still has two release-process caveats: there is no Git commit yet, so a literal clone/archive-of-HEAD test is impossible, and the disposition of generated CPU evidence must be decided after the final demo. Those points are detailed under Remaining issues.
...
1. **No immutable repository revision yet.** `git status` reports the project as untracked because the repository has no first commit. Consequently, `git clone`, `git archive HEAD`, commit-based evidence provenance and a true clean-tree release build cannot be validated. The Docker build stage, independent wheel environment and sdist build provide strong substitutes, but the integration owner must create a revision and rerun `make check`/`make demo` against it.
2. **Generated evidence retention needs one final decision.** `.gitignore` excludes `artifacts/demo` and `reports/demo`, while several component documents call the example run “checked-in.” Current pre-regeneration artifacts contain absolute developer paths and their old `models.json` no longer validates against newly strengthened calibration fields, so a report-regeneration probe correctly fails. Source now emits repository-relative URIs and CI uploads evidence, but after the final run the owner must either commit the regenerated, secret-scanned evidence/reports or change all “checked-in” claims to “generated/CI-retained.” Stale output must not be committed.
