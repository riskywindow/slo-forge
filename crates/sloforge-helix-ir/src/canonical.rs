//! Canonical JSON serialization compatible with the Python Helix boundary.
...
/// # Errors
///
/// Returns an error for values JSON cannot represent.
...
/// Compute lowercase SHA-256 over canonical JSON.
///
/// # Errors
///
...
pub fn canonical_hash<T: Serialize>(document: &T) -> Result<String, ProtocolError> {
    Ok(format!("{:x}", Sha256::digest(canonical_json(document)?)))
}
