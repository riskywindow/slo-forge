...
use crate::{ArtifactDigest, DigestAlgorithm, ProtocolError};
...
/// Serialize a document using the cross-language canonical JSON profile.
///
/// # Errors
///
...
/// Compute the stable SHA-256 content digest of canonical JSON.
///
/// # Errors
///
/// Returns an error when the value cannot be represented as JSON.
pub fn canonical_hash<T: Serialize>(document: &T) -> Result<ArtifactDigest, ProtocolError> {
    let value = format!("{:x}", Sha256::digest(canonical_json(document)?));
    Ok(ArtifactDigest {
